...
// √Ë ˆ
//
// ±æŒƒº˛ «DTUÕ®—∂Õ¯πÿ◊∞÷√µƒIEC60870-5-104πÊ‘º¥¶¿Ì≥Ã–Ú
// ±‡–¥»À:R&N
//email           :R&N
//  »’	   ∆⁄:2014.9.16
//  ◊¢	    Õ:
...
static BufStruct 			* pBufRecvRoot;		//πÊ‘ºª∫¥ÊΩ·ππ∏˘÷∏’Î
static BufStruct 			* pBufRecvFill;		//πÊ‘ºª∫¥ÊΩ·ππœ¬“ª∏ˆ“™ÃÓ≥‰µƒΩ·ππ÷∏’Î
static BufStruct 			* pBufRecvGet;		//πÊ‘ºª∫¥ÊΩ·ππœ¬“ª∏ˆ“™∂¡»°µƒΩ·ππ÷∏’Î
static int 		  		    new_fd=-1;
static int 		   		    sock_fd=-1;
//sem_t              		    sem_iec104;
static LOCAL_Module                * sys_mod_p;

static YXInfoStruct         * pYXInfoRoot;     //”√”⁄“£–≈±‰Œªº«¬º
static YXInfoStruct         * pYXInfoFill;
static YXInfoStruct         * pYXInfoGet;

static SOEInfoStruct        * pSOERoot;         //”√”⁄“£–≈SOE ¬º˛
...
static void IEC104_AP_RP(IEC104_ASDU_Frame *pRecv_ASDUFrame,unsigned char ucTemp_Length) ;//¥À¥¶≥§∂»Œ™ASDU≥§∂»
...
static void IEC104_AP_YK(IEC104_ASDU_Frame *pRecv_ASDUFrame,unsigned char ucTemp_Length, unsigned char flag) ;//¥À¥¶≥§∂»Œ™ASDU≥§∂»
...
static void IEC104_AP_IC(IEC104_ASDU_Frame *pRecv_ASDUFrame , unsigned char ucTemp_Length); //¥À¥¶≥§∂»Œ™ASDU≥§∂»
static void IEC104_AP_RQALL_CON(IEC104_ASDU_Frame *pRecv_ASDUFrame);
static unsigned char IEC104_AP_ALL_YX( void );// ±®∏Ê»´≤ø“£≤‚ ˝æ›
static unsigned char IEC104_Process_COS( void ) ;//”¶”√≤„¥¶¿Ì
static unsigned char IEC104_Process_SOE( void ); //”¶”√≤„¥¶¿Ì
...
/***************************************************************************/
//∫Ø ˝: static   void IEC104_Creat_connection( void )
//Àµ√˜:”√”⁄Õ¯¬Á¡¨Ω”£¨π©iec104πÊ‘º π”√
// ‰»Î:
// ‰≥ˆ:
//±‡º≠:R&N
// ±º‰:2015.4.21
...
static   void IEC104_Creat_connection( void )
{
	...
	setsockopt( sock_fd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on) );//‘ –Ìµÿ÷∑¡¢º¥ π”√

	 strncpy(ifr.ifr_name, "eth0" , IFNAMSIZ);
	 if (ioctl(sock_fd, SIOCGIFADDR, &ifr) == 0)   //SIOCGIFADDR ªÒ»°ip address
	 {
	...
	while (1)
	{
		...
		while (1)
		{
			FD_ZERO(&rfds);		// ∞—ºØ∫œ«Âø’
			FD_SET(0, &rfds);		// ∞—±Í◊º ‰»Îæ‰±˙ 0 º”»ÎµΩºØ∫œ÷–
			FD_SET(new_fd, &rfds);// ∞—µ±«∞¡¨Ω”æ‰±˙ new_fd º”»ÎµΩºØ∫œ÷–
			maxfd = 0;
			if (new_fd > maxfd)
				maxfd = new_fd;
			tv.tv_sec = 1;			// …Ë÷√◊Ó¥Ûµ»¥˝ ±º‰
			tv.tv_usec = 0;

			retval = select(maxfd +1, &rfds, NULL, NULL, &tv);// ø™ ºµ»¥˝
			...
			else if (retval == 0)  //≥¨ ±µ»¥˝
			{
			 ...
			 {
				if (FD_ISSET(0, &rfds)) 					//÷–∂œ ‰»Î ˝æ›
				{
				}
				if (FD_ISSET(new_fd, &rfds))
				{
					bzero(pBufRecvFill->buf, IEC104BUFDOWNLENGTH);
					len = recv(new_fd, pBufRecvFill->buf, IEC104BUFDOWNLENGTH, 0);		// Ω” ’øÕªß∂Àµƒœ˚œ¢
					if (len > 0)
					{
...
/***************************************************************************/
//∫Ø ˝: void Init_IEC104_Buf(void)
//Àµ√˜:≥ı ºªØª∫≥Â£¨∞¸¿® ’µΩ ˝æ›±£¥Êµƒª∫≥Â£¨≤¢Ω´¡¨Ω”≥…¡¥±Ì
// ‰»Î:Œﬁ
// ‰≥ˆ:Œﬁ
/***************************************************************************/
...
//	pointer = malloc (IEC104BUFUPLENGTH);	//ø™±Ÿø’º‰”√”⁄πÊ‘º∑¢ÀÕ ˝æ›£¨≥§∂»256
//     	if (pointer ==NULL)
//	{
//		my_debug("pBufRecvRoot malloc fail\n");
//		goto leave4;
...
/***************************************************************************/
//∫Ø ˝:void IEC104_Process_Message(msg->cmd, msg->buf, UInt32 buf)
//Àµ√˜:¥¶¿ÌMESSAGEµƒ∫Ø ˝
// ‰»Î:messageµƒ√¸¡Ó∫Õbuf“‘º∞data£¨∆‰÷–
// ‰≥ˆ:
//±‡º≠:R&N
// ±º‰:2015.9.23
/***************************************************************************/
void IEC104_Process_Message(UInt32 cmd, UInt32 buf, UInt32 data)
{
    ...
    switch (cmd&0xFFFF)
    {
        case MSG_COS:
            for(i=0; i<32; i++)
            {
                if(buf&(0x01<<i))//±Ì æµ⁄(cmd>>16)*32+iÕ®µ¿±‰Œª
                {
...
//             my_debug("send SOE number :%d",cout_framer);
            for(i=0; i<32; i++)
            {
                if(buf&(1<<i))//±Ì æµ⁄(cmd>>16)*32+iÕ®µ¿±‰Œª
                {
            ...
            if(cout_framer>=100)				//√ø∏ˆlog¿Ô√Ê◊Ó∂‡¥Ê¥¢∂‡…Ÿ∏ˆº«¬ºs
            {  
                cout_framer=0;
...
//	free(pIEC104_Struct->pSendBuf);
    ...
    */
	...
	sem_destroy(&sem_soe104);//œ˙ªŸSOE–≈∫≈¡ø
//	sem_destroy(&sem_iec104);
	return 0;
}
/***************************************************************************/
//∫Ø ˝: void *Network_Thread(void * arg)
//Àµ√˜:”√”⁄Õ¯¬Á¡¨Ω”£¨π©iec104πÊ‘º π”√
// ‰»Î:
// ‰≥ˆ:
//±‡º≠:R&N
...
void  *Network_Thread( void * arg  )
{
	...
	sem_init(&sem_soe104, 0, 0);//¥¥Ω®“ª∏ˆŒ¥√¸√˚µƒ–≈∫≈¡ø
...
/************************************************************************************/
//∫Ø ˝:static int  Get_Parp_From_Line(char *line, IEC104Struct   * item)
//Àµ√˜: Ω‚Œˆ≈‰÷√∫Ø ˝÷–µƒ√ø“ª––£¨Ω‚ŒˆπÊ‘º≈‰÷√Œƒº˛
// ‰»Î:line       √ø“ª––
// ‰≥ˆ:: 1∑µªÿ’˝»∑Ω·π˚       0 Ω‚ŒˆΩ· ¯
/************************************************************************************/
static int  Get_Parp_From_Line(char *line, IEC104Struct   * item)
...
//		my_debug("item->infoaddlen=%d",item->infoaddlen);
	...
	tmp_line= NULL;
	tmp_line=strstr((const char *)line, "yctype="); //¥´µ›÷µµ√¿‡–Õ
	if(tmp_line!=NULL)
	{
		token = strtok( tmp_line, "#");
...
/***************************************************************************/
//∫Ø ˝Àµ√˜:∂¡»°104≤Œ ˝    ∂¡»°πÊ‘ºœ‡πÿµƒ≈‰÷√
//
// ‰»Î:fd ¬∑æ∂√˚≥∆“‘º∞Œƒº˛item ≤Œ ˝÷∏’Î
// ‰≥ˆ:Œﬁ
//±‡º≠:R&N
// ±º‰:2014.10.08
/***************************************************************************/
static void IEC104_Read_Conf(char * fd,  struct _IEC104Struct   * iec104)
{
	 ...
	 fseek(fp, 0, SEEK_SET); 			//÷ÿ–¬∂®“ÂµΩø™ ºŒª÷√
...
/***************************************************************************/
//∫Ø ˝: static unsigned char Init_Protocol_IEC104(void)
//Àµ√˜:”√”⁄≥ı ºªØIEC104
// ‰»Î:   Œﬁ
// ‰≥ˆ:  Œﬁ
//±‡º≠:R&N
...
static unsigned char IEC104_Init_Protocol(void)
{
	...
	pIEC104_Struct->usRecvNum = 0;			//Ω” ’–Ú¡–∫≈
	pIEC104_Struct->usSendNum = 0;			//∑¢ÀÕ–Ú¡–∫≈
...
/**************************************************************************/
//∫Ø ˝:static void IEC104_Protocol_Entry(  void  )
//Àµ√˜:104πÊ‘ºµƒ»Îø⁄
// ‰»Î:Œﬁ
// ‰≥ˆ:Œﬁ
//±‡º≠:R&N
...
static void IEC104_Protocol_Entry(  void  )
{
	...
	if(pBufRecvGet->usStatus == FULL)// ’µΩ ˝æ›
	{
		...
		ucTemp_ReturnFlag = IEC104_VerifyDLFrame(pTemp_Buf, usTemp_Length);   //—È÷§104÷–÷° «I  U  S∏Ò Ω
//		my_debug("ucTemp_ReturnFlag:%d\n", ucTemp_ReturnFlag);
		if( (ucTemp_ReturnFlag == IEC104FRAME_I) || (ucTemp_ReturnFlag == IEC104FRAME_U))
		{
			IEC104_ProcessAPDU(pTemp_Buf, usTemp_Length);					//¥¶¿Ì104πÊ‘º÷–µƒ ˝æ›
	...
	IEC104_UnsoSend(); // ¬º˛…œ¥´  SOE   COSµ»
}


/****************************************************************************/
//∫Ø ˝Àµ√˜:¥¶¿Ì¥¯ ±±Íµƒ“£øÿ√¸¡Ó   ¥¯ ±±Íµ•µ„“£øÿ
// ‰»Î:ASDU÷°     ∫Õ≥§∂»
//±‡º≠:R&N
// ±º‰:2014.10.10
//YK_BISUO				0
...
static void IEC104_AP_YK(IEC104_ASDU_Frame *pRecv_ASDUFrame,unsigned char ucTemp_Length, unsigned char flag) //¥À¥¶≥§∂»Œ™ASDU≥§∂»
{
	...
	if(flag==WITHTIME)
	{
		...
		if(((timenow>timerecv)&&(timenow-timerecv>15))||((timerecv>timenow)&&(timerecv-timenow>15)))//»Áπ˚≥¨π˝15√ÎæÕ≤ªƒ‹∑÷∫œ’¢
		{
	...
	if(pRecv_ASDUFrame->ucInfoUnit[3] & 0x80) //“£øÿ—°‘Ò
	{
		if(pRecv_ASDUFrame->ucCauseL == IEC104_CAUSE_ACT) //º§ªÓ
		{
			switch(IEC104_AP_YKSel(pRecv_ASDUFrame))
			{
				case YK_NORMAL_CONTRL: 			//“ª∞„“£øÿ—°‘Ò
					IEC104_AP_PositiveCON(pRecv_ASDUFrame , ucTemp_Length);
					break;
				case YK_INVALID_ADDR: 			//–≈œ¢ÃÂµÿ÷∑”–ŒÛ
				    my_debug("YK_INVALID_ADDR is wrong");
					IEC104_AP_UnknownInfoAddr(pRecv_ASDUFrame,ucTemp_Length);
					break;
			}
		}
		else if(pRecv_ASDUFrame->ucCauseL == IEC104_CAUSE_DEACT) //Õ£÷πº§ªÓ»∑»œ
		{
			switch(IEC104_AP_YKSelHalt(pRecv_ASDUFrame))
			{
				case YK_CANCEL_SEL:			 //“ª∞„“£øÿ—°‘Ò≥∑œ˙
					IEC104_AP_NegativeCON(pRecv_ASDUFrame , ucTemp_Length);
					break;
				case YK_INVALID_ADDR:		 //–≈œ¢ÃÂµÿ÷∑”–ŒÛ
					IEC104_AP_UnknownInfoAddr(pRecv_ASDUFrame,ucTemp_Length);
...
static unsigned char IEC104_AP_YKExec(IEC104_ASDU_Frame * pRecv_ASDUFrame, unsigned char ucTemp_Length)
{
	...
	if((pRecv_ASDUFrame->ucInfoUnit[3]&0x01)==0X01)   //’‚¿ÔÃÌº”“£øÿ∑÷’¢
	{
		if(Point_Info->usFlag &(1<<8))			           //“—æ≠—°÷–
		{
				Point_Info->usFlag &=~(1<<8); 		   //∫œ’¢∂Ø◊˜,≤¢«“Ω‚≥˝—°÷–
				//ÃÌº”∫œ’¢∂Ø◊˜
	...
	{
		if(Point_Info->usFlag &(1<<9))			           //“—æ≠—°÷–
		{
				Point_Info->usFlag &=~(1<<9); 		   //∑÷’¢∂Ø◊˜,≤¢«“Ω‚≥˝—°÷–
				//ÃÌº”∑÷’¢∂Ø◊˜
				Send_Event_By_Message(MSG_YK, Point_Info->uiOffset, YK_OPEN_EXECUTIVE);
		}
	}
	if(1)											   //∂Ø◊˜≥…π¶ ?    ÷¥––µƒ ±∫Ú±ÿ–Î≈–∂œ—°÷–√ª”–
...
static unsigned char IEC104_AP_YKSelHalt(IEC104_ASDU_Frame * pRecv_ASDUFrame)
{
	...
	if((pRecv_ASDUFrame->ucInfoUnit[3]&0x01)==0X01)//’‚¿ÔÃÌº”“£øÿ∑÷’¢—°÷–»°œ˚  ªπ «“£øÿ∫œ’¢—°÷–»°œ˚
	{
...
static unsigned char IEC104_AP_YKSel(IEC104_ASDU_Frame * pRecv_ASDUFrame)
{
	...
	if((pRecv_ASDUFrame->ucInfoUnit[3]&0x01)==0X01)//’‚¿ÔÃÌº”“£øÿ∑÷’¢—°÷–  ªπ «“£øÿ∫œ’¢—°÷–
	{
		Point_Info->usFlag |=(1<<8);
        Send_Event_By_Message(MSG_YK, Point_Info->uiOffset, YK_OPEN_SEL);//◊Ó∏ﬂŒª «1 «±Ì æ—°÷–£¨ «0±Ì æ÷¥––
    }
	else
    {
		Point_Info->usFlag |=(1<<9);
...
//			IEC104_AP_RD(pTemp_ASDUFrame,ucTemp_Length-6);
	...
	else if( (usTemp_ServSendNum & 0x0002) != 0 ) //FORMAT_U
	{
		if(usTemp_ServSendNum & 0x04)
		{
			...
			pIEC104_Struct->ucDataSend_Flag = TRUE;  // «∑Ò‘ –Ì∑¢ÀÕµƒ±Íº«
		}
...
static void IEC104_AP_IC(IEC104_ASDU_Frame *pRecv_ASDUFrame , unsigned char ucTemp_Length) //¥À¥¶≥§∂»Œ™ASDU≥§∂»
{
	...
	//◊‹’ŸªΩŒ™º§ªÓ£¨∑÷◊È’ŸªΩŒ™«Î«Û£¨ªπ”–Õ£÷πº§ªÓ£¨‘› ±≤ªøº¬«
	...
	if(ucTemp_GroupNo == 20)  //◊‹’ŸªΩ
	{
		pIEC104_Struct->ucInterro_First = 0x01; //ªÿ¥◊‹’ŸªΩ»∑»œ±Í÷æ
		IEC104_AP_RQALL_CON(pRecv_ASDUFrame);
		return;
	}
//	else if(ucTemp_GroupNo >= 21 && ucTemp_GroupNo <= 28 )	// “£–≈◊È 21~28
//	{
...
//		return;
//	}
//	else if( ucTemp_GroupNo >= 29 && ucTemp_GroupNo <= 32 )		// “£≤‚◊È 29~32
//	{
...
static void IEC104_AP_RQALL_CON(IEC104_ASDU_Frame *pRecv_ASDUFrame)
{
	...
	pTemp_ASDUFrame->ucType = pRecv_ASDUFrame->ucType;
	pTemp_ASDUFrame->ucRep_Num = 0x01;  // ˝æ›∏ˆ ˝≥ı÷µ
	...
	*pTemp_InfoPointer = pRecv_ASDUFrame->ucInfoUnit[3];
	...
	pTemp_ASDUFrame->ucCauseL=IEC104_CAUSE_ACTCON;  //07-º§ªÓ»∑»œ
...
static void IEC104_AP_CS(unsigned char *pTemp_Buf,unsigned char ucTemp_Length)
{
	...
	if(ucSec>59)
		return;
	usUSec = usMSec%1000; //’‚∏ˆ «ms
	usUSec = usUSec *1000; //’‚∏ˆ «us
	ucMin = buf[2]&0x3f;
	if(ucMin>59)
		return;
	...
	Set_Rtc_Time(stime,  usUSec);	//…Ë÷√RTC ±º‰
...
static void IEC104_AP_INTERROALL_END( void )
{
	...
	pTemp_ASDUFrame->ucCauseL = IEC104_CAUSE_ACTTERM; 	//0x0a-º§ªÓΩ· ¯
	pTemp_ASDUFrame->ucCauseH = 0x00;
	...
	IEC104_APDU_Send(FORMAT_I, len+6);					//len+6Œ™FORMAT_IµƒASDU≥§∂»
}
/**************************************************************************/
//∫Ø ˝√Ë ˆ:±®∏ÊÀ˘”–“£≤‚ ˝æ›,∑¢ÀÕ∏°µ„ ˝
// ‰»Î:Œﬁ
...
//±‡º≠:R&N
/**************************************************************************/
static unsigned char IEC104_AP_ALL_YC( void )// ±®∏Ê»´≤ø“£≤‚ ˝æ›
{
	...
	if(sys_mod_p->Continuity & YC_NOTCONTINUE) 			 		//µÿ÷∑≤ª¡¨–¯
	...
	pTemp_ASDUFrame->ucCauseH = 0x00;
	pTemp_ASDUFrame->ucCommonAddrL = pIEC104_Struct->linkaddr;    //’æµÿ÷∑
	pTemp_ASDUFrame->ucCommonAddrH = 0x00;

	len=0;
	pTmp_Point = sys_mod_p->pYC_Addr_List_Head;

	for(k=0; k<(sys_mod_p->usYC_Num-send_over); )		//∑¢ÀÕ∂‡…Ÿ¥Œ
	{
		len = 0;
		...
		{
			if(sys_mod_p->Continuity & YC_NOTCONTINUE) 					//≤ª¡¨–¯µÿ÷∑
			{
				pTemp_ASDUFrame->ucInfoUnit[len++] = (unsigned char)(pTmp_Point->uiAddr&0xff);
				pTemp_ASDUFrame->ucInfoUnit[len++] = (unsigned char)(pTmp_Point->uiAddr>>8);
				pTemp_ASDUFrame->ucInfoUnit[len++] =  (unsigned char)(pTmp_Point->uiAddr>>16);
			}
			data =(float)Read_From_Sharearea(pTmp_Point->uiOffset, TYPE_YC);	  //“£≤‚÷µ
			//my_debug("pTmp_Point->uiOffset:%d",pTmp_Point->uiOffset);
            if(pIEC104_Struct->yctype == TYPE_FLOAT)//∏°µ„÷µ
            {
    			memcpy((UInt8 *)(pTemp_ASDUFrame->ucInfoUnit+len), (UInt8 *)&data,4);
    			len+=4;
            }
            else if(pIEC104_Struct->yctype == TYPE_GUIYI)//πÈ“ª÷µ
            {
            	pTemp_ASDUFrame->ucType = IEC104_M_ME_NA_1;
    			pTemp_ASDUFrame->ucInfoUnit[len++] = 0xff;
    			pTemp_ASDUFrame->ucInfoUnit[len++] = 0xff;
            }
			pTemp_ASDUFrame->ucInfoUnit[len++] = 0x00;	//∆∑÷ “Ú ˝
		...
		if(sys_mod_p->Continuity & YC_NOTCONTINUE)   //≤ª¡¨–¯
...
static unsigned char IEC104_AP_ALL_YX( void )// ±®∏Ê»´≤ø“£–≈ ˝æ›
{
	...
	pTemp_ASDUFrame->ucType = IEC104_M_SP_NA_1;					//µ•µ„“£–≈
	if(sys_mod_p->Continuity & YC_NOTCONTINUE) 			 		//µÿ÷∑≤ª¡¨–¯
		pTemp_ASDUFrame->ucRep_Num = 0;
	else
		pTemp_ASDUFrame->ucRep_Num = IEC104_VSQ_SQ_BIT;
	...
	for(k=0; k<(sys_mod_p->usYX_Num-send_over); )		//∑¢ÀÕ∂‡…Ÿ¥Œ
	{
		len = 0;
		...
		{
			if(sys_mod_p->Continuity & YX_NOTCONTINUE) 					//≤ª¡¨–¯µÿ÷∑
			{
				pTemp_ASDUFrame->ucInfoUnit[len++] = (unsigned char)(pTmp_Point->uiAddr&0xff);
				pTemp_ASDUFrame->ucInfoUnit[len++] = (unsigned char)(pTmp_Point->uiAddr>>8);
				pTemp_ASDUFrame->ucInfoUnit[len++] =  (unsigned char)(pTmp_Point->uiAddr>>16);
			}
			data = Read_From_Sharearea(pTmp_Point->uiOffset, TYPE_YX_STATUS_BIT);	  //“£–≈÷µ
			if((data&(1<<(pTmp_Point->uiOffset&0x7))) !=0)
					pTemp_ASDUFrame->ucInfoUnit[len++] = 0x01;			//∆∑÷ “Ú ˝
				else
					pTemp_ASDUFrame->ucInfoUnit[len++] = 0x0;				//∆∑÷ “Ú ˝
			if(pTmp_Point->next !=NULL)
				pTmp_Point = pTmp_Point->next;
		}
		send_over = send_over +j;
		my_debug("usYX_Num:%d send_over:%d j:%d", sys_mod_p->usYX_Num, send_over, j);
		if(sys_mod_p->Continuity & YX_NOTCONTINUE)   //≤ª¡¨–¯
			pTemp_ASDUFrame->ucRep_Num  = j;
		else
			pTemp_ASDUFrame->ucRep_Num  =IEC104_VSQ_SQ_BIT|j;
		IEC104_APDU_Send(FORMAT_I, len+6);
...
/***********************************************************************/
//∫Ø ˝:static unsigned char IEC104_Chg_Search()
//Àµ√˜:ºÏ≤È «∑Ò”–œ‡πÿµƒ ¬º˛
// ‰»Î:Œﬁ
// ‰≥ˆ:∏˜÷÷ ¬º˛
//±‡º≠:R&N
...
/***********************************************************************/
//∫Ø ˝:static unsigned char IEC104_Process_SOE( void ) //”¶”√≤„¥¶¿Ì
//Àµ√˜:¥¶¿Ìµ•µ„“£–≈SOE
// ‰»Î:
// ‰≥ˆ:
//±‡º≠:R&N
...
static unsigned char IEC104_Process_SOE( void ) //”¶”√≤„¥¶¿Ì
{
	...
	pTemp_ASDUFrame = (IEC104_ASDU_Frame *)(pIEC104_Struct->pSendBuf+6);
	pTemp_ASDUFrame->ucType = IEC104_M_SP_TB_1;         //  ˝æ›¿‡–Õ
	pTemp_ASDUFrame->ucRep_Num = 0x00;                  // ˝æ›∏ˆ ˝≥ı÷µ
	pTemp_ASDUFrame->ucCauseL = IEC104_CAUSE_SPONT;     // ∑¢ÀÕ‘≠“Ú
	pTemp_ASDUFrame->ucCauseH = 0x00;
	pTemp_ASDUFrame->ucCommonAddrH = 0x00;
    ...
    while(pSOEGet->ucStatus == FULL)
    {
        ...
        if(pTmp_Point != NULL)
        {
            ...
            if(pSOEGet->ucYXValue)
      		    pTemp_ASDUFrame->ucInfoUnit[len++] = 1;   //÷µ
      		else
                pTemp_ASDUFrame->ucInfoUnit[len++] = 0;   //÷µ
	...
	{
		IEC104_APDU_Send(FORMAT_I,len+6);//len+5Œ™FORMAT_IµƒASDU≥§∂»
		return(1);
	}
	else
		return(0);
...
/***********************************************************************/
//∫Ø ˝:static unsigned char IEC104_Process_COS( void ) //”¶”√≤„¥¶¿Ì
//Àµ√˜:¥¶¿Ìµ•µ„“£–≈±‰Œª
// ‰»Î:
// ‰≥ˆ:
//±‡º≠:R&N
...
static unsigned char IEC104_Process_COS( void ) //”¶”√≤„¥¶¿Ì
{
	...
	pTemp_ASDUFrame = (IEC104_ASDU_Frame *)(pIEC104_Struct->pSendBuf+6);
	pTemp_ASDUFrame->ucType = IEC104_M_SP_NA_1;         //  ˝æ›¿‡–Õ
	pTemp_ASDUFrame->ucRep_Num = 0x00;                  // ˝æ›∏ˆ ˝≥ı÷µ
	pTemp_ASDUFrame->ucCauseL = IEC104_CAUSE_SPONT;     // ∑¢ÀÕ‘≠“Ú
	pTemp_ASDUFrame->ucCauseH = 0x00;
	pTemp_ASDUFrame->ucCommonAddrH = 0x00;
...
/**************************************************************************/
//∫Ø ˝Àµ√˜:”…”⁄104»°œ˚¡À’ŸªΩ“ªº∂∂˛º∂ ˝æ›£¨‘› ±Œﬁ∑«∆Ω∫‚¥´ ‰∑Ω Ωœ¬µƒ ¬º˛ ’ºØ∑Ω∑®£¨
//			      À˘“‘≤…”√∆Ω∫‚¥´ ‰∑Ω Ωœ¬µƒ ¬º˛ ’ºØ∑Ω∑®£¨ ¬º˛»´≤ø÷˜∂Ø…œÀÕ
// ‰≥ˆ:Œﬁ
//±‡º≠:R&N
/**************************************************************************/
...
static void IEC104_UnsoSend( void )
{
	...
	if(ucTemp_Chg==MSG_COS)           //YX change  		2---“£–≈±‰Œª
	{
...
//		pIEC104_Struct->ucYK_TimeCount++;
	...
	if(pIEC104_Struct->ucInterro_First)		//◊‹’ŸªΩ±Íº«
	{
		IEC104_AP_INTERROALL();
		return;
	}

//	for(i=0;i<pIEC104_Struct->ucRTU_Num;i++) //≤È’“∏˜RTU «∑Ò¥Ê‘⁄“£≤‚◊È’ŸªΩ
//	{
...
//
	...
	if(ucTemp_Chg == MSG_SOE)		 //	4---SOE ¬º˛
	{
		if(IEC104_Process_SOE())
			return;
	}
	if(ucTemp_Chg == 1)          //YC Overstep   1---“£≤‚ ˝æ›
	{
...
/**************************************************************************/
//∫Ø ˝π¶ƒ‹:ºÏ—ÈΩ” ’µΩµƒ ˝æ› «≤ª «104πÊ‘ºµƒ±Í÷æ÷°
// ‰»Î:pTemp_BufΩ” ’µΩµƒ ˝æ›ª∫¥Ê         usTemp_LengthΩ” ’µΩµƒ≥§∂»
// ‰≥ˆ:
//		IEC104FRAMEERR         1	¥ÌŒÛ÷°
//		IEC104FRAMESEQERR    2	–£—È¥ÌŒÛ
//		IECFRAMENEEDACK       3	–Ë“™ªÿ∏¥
//		IEC104FRAMEOK          0	OK
/**************************************************************************/
static unsigned char IEC104_VerifyDLFrame(unsigned char * pTemp_Buf , unsigned short usTemp_Length)
{
	...
	usTemp_ServSendNum = (pTemp_Buf[3]<<8)|pTemp_Buf[2];  //∑¢ÀÕ–Ú¡–∫≈
	usTemp_ServRecvNum = (pTemp_Buf[5]<<8)|pTemp_Buf[4];  //Ω” ’∫≈
	my_debug("usTemp_ServSendNum:%d usTemp_ServRecvNum:%d",usTemp_ServSendNum,usTemp_ServRecvNum);
	if( !(usTemp_ServSendNum & 0x0001) )   //FORMAT_I
	{
		if(ucTemp_Length < 12)              //ASDU I ÷°”¶∏√¥ÔµΩ“ª∏ˆ∆ º≥§∂»
		{
			return IEC104FRAMEERR;
		}
		...
		if(pIEC104_Struct->ucSendCountOverturn_Flag)//∑¢ÀÕI∏Ò Ω÷°¥ÔµΩ0x7FFF£¨Ω¯––∑¥◊™
		{
		...
		if( usTemp_ServSendNum != pIEC104_Struct->usRecvNum )//÷˜’æ∑¢ÀÕ–Ú¡–∫≈  != ¥”’æΩ” ’–Ú¡–∫≈
			return IEC104FRAMESEQERR;
		else                                                  //÷˜’æ∑¢ÀÕ–Ú¡–∫≈  == ¥”’æΩ” ’–Ú¡–∫≈
		{
			pIEC104_Struct->w++;                                //Ω” ’∑Ω¡¨–¯Ω” ’W÷°ªÿ∏¥»∑»œ
            //pIEC104_Struct->usRecvNum = (pIEC104_Struct->usRecvNum+1)&0x7FFF;//◊Ó¥Û0x7FFF
			...
			if(pIEC104_Struct->w >= IEC104_MAX_W)               //Ω” ’µΩIEC104_MAX_W∏ˆI±®Œƒ∫Û∏¯≥ˆ»∑»œ
			{
...
//	else if( (usTemp_ServSendNum & 0x03)==0x01 )   //FORMAT_S
    else if( !(usTemp_ServSendNum & 0x0002))   //FORMAT_S
	{
		...
		else
		{
			...
			else if( usTemp_ServRecvNum > pIEC104_Struct->usSendNum );//÷˜’æ
				//return IEC104FRAMESEQERR;
		}
		return IECFRAMENEEDACK;
	}
		...
		if(pIEC104_Struct->ucDataSend_Flag)
		{
			...
			*pTemp_SendBuf = usTemp_Num >> 8;
            ...
            if(flag_kvalue==0)
            {
                ...
                if(cout_Iframe<20)
                {
                    if(pIEC104_Struct->k >= IEC104_MAX_K)//∏„«Â≥˛
            		{
                         my_debug("k max 12...");
                         close(new_fd);
                         //return ;
            ...
            if(ucTemp_nSendCount != ucTemp_Value+6)//»Áπ˚∑¢ÀÕ≤ª≥…π¶
			{
...
//			pIEC104_Struct->ucWaitServConCount=0;
	  ...
	  case FORMAT_S:
		...
		*pTemp_SendBuf = usTemp_Num >> 8;
		...
		pIEC104_Struct->w = 0;//Ω” ’∑Ω¡¨–¯Ω” ’W÷°ªÿ∏¥»∑»œ
		pIEC104_Struct->k = 0;//Ω” ’∑Ω¡¨–¯Ω” ’K÷°ªÿ∏¥»∑»œ
        //pIEC104_Struct->usSendNum=0;
        flag_kvalue++;
...
//        cout_Iframe=0;
      ...
      //return IECFRAMENEEDACK;
      //return;
      //»Áπ˚ «œ»∏¯◊”Ω¯≥Ã∑¢–≈∫≈Ω· ¯µÙÀ¸√«£¨»ª∫Û◊‘º∫exit(0)ÕÀ≥ˆ
      //¡Ì“ª÷÷∑Ω∑®ø…“‘…Ë“ª∏ˆ»´æ÷±‰¡ø£¨√ø¥ŒΩ¯»Î–≈∫≈¥¶¿Ì∫Ø ˝ ±º”1
}
void set_timer()  
{
    ...
    //struct itimerval timer;
    //…Ë÷√ ±º‰º‰∏ÙŒ™10√Î
/*  interval.tv_sec = pIEC104_Struct->ucTimeOut_t2;//√Î 
    interval.tv_usec=0; //Œ¢√Î
    timer.it_interval = interval;//∂® ±∆˜∆Ù∂Ø∫Û£¨√ø∏Ùx√ÎΩ´÷¥––œ‡”¶µƒ∫Ø ˝
    timer.it_value = interval;// Æ√Î÷”∫ÛΩ´∆Ù∂Ø∂® ±∆˜
*/    
    timer.it_value.tv_sec = 0;  // Æ√Î÷”∫ÛΩ´∆Ù∂Ø∂® ±∆˜
    timer.it_value.tv_usec = 500;
    timer.it_interval.tv_sec  =0; //∂® ±∆˜∆Ù∂Ø∫Û£¨√ø∏Ù0√ÎΩ´÷¥––œ‡”¶µƒ∫Ø ˝
    timer.it_interval.tv_usec = 0;
    setitimer(ITIMER_VIRTUAL, &timer, 0);//»√À¸≤˙…˙SIGVTALRM–≈∫≈
}
void uninit_time() 
{ 
    //struct itimerval timer;
...
/**************************************************************************/
//∫Ø ˝:void IEC104_Senddata2pc(int fd, char *tmp_buf, UInt16 len)
//Àµ√˜:”√”⁄∑¢ÀÕº‡øÿ ˝æ›µΩPC
// ‰»Î:fd”√¿¥≈–∂œ «iec101ªπ «iec104  tmp_buf–Ë“™∑¢ÀÕµƒ ˝æ›  len–Ë“™∑¢ÀÕµƒ ˝æ›≥§∂»
// ‰≥ˆ:Œﬁ
//±‡º≠:R&N
/**************************************************************************/
...
void PC_Send_YC(UInt8* buf, UInt8 len)
{
    ...
    UInt16 uindex;
    if((buf[4]== 0xFF)&&(buf[5] == 0xFF))//’ŸªΩÀ˘”–µƒ“£≤‚
    {
        k = 0;
        pTmp_Point = sys_mod_p->pYC_Addr_List_Head;
        while(pTmp_Point != NULL)
        {
            ...
            //tmp_buf[k] =   pTmp_Point->uiOffset&0xFF;       //’º”√2◊÷Ω⁄
            //tmp_buf[k+1] =   (pTmp_Point->uiOffset>>8)&0xFF;
            //tmp_buf[k+2] =   pTmp_Point->uiAddr&0xFF;       //’º”√4∏ˆ◊÷Ω⁄
            //tmp_buf[k+3] =   (pTmp_Point->uiAddr>>8)&0xFF;
            //tmp_buf[k+4] =   0x00;
            //tmp_buf[k+5] =   0x00;
            data =(float)Read_From_Sharearea(pTmp_Point->uiOffset, TYPE_YC);//“£≤‚÷µ
//          memcpy((UInt8 *)&tmp_buf[k+6], (UInt8 *)&data,4);//’º”√4∏ˆ◊÷Ω⁄
            memcpy((UInt8 *)&tmp_buf[k+2], (UInt8 *)&data,4);//’º”√4∏ˆ◊÷Ω⁄
            pTmp_Point = pTmp_Point->next;
//            k = k +10;
            k = k +6;
//            if(k>55)
            if(k>3)
            {
                ...
                //my_debug("tem_buf[k+6]:%f tem_buf[k+10]:%f",tem_buf[k+6],tem_buf[k+10])£ª
                k = 0;
            }
        }
        if(k>9)
    ...
    else
    {
        ...
        for(i=0; i<len-5; )
        {
           ...
           while(pTmp_Point != NULL)
            ...
            i= i+2;//√ø∏ˆindex’º”√2∏ˆ◊÷Ω⁄
            if(pTmp_Point != NULL)//’“µΩ¡À∂‘”¶µƒΩ·ππÃÂ
            {
...
void PC_Send_YX(UInt8* buf,UInt8 len)
{
    ...
    if(buf[4] == 0xFF)//’ŸªΩÀ˘”–µƒ“£–≈
    {
        ...
        while(pTmp_Point != NULL)
        {
            ...
            //tmp_buf[k+3] = 0;
            //tmp_buf[k+4] = 0;
            data =Read_From_Sharearea((pTmp_Point->uiOffset)-1, TYPE_YX_STATUS_BIT);	  //“£–≈÷µ
    ...
    else
    {
        ...
        for(i=0; i<len-5; i++)
        {
           ...
           while(pTmp_Point != NULL)
            ...
            //tmp_buf[k+3] = 0;
            //tmp_buf[k+4] = 0;
            data =Read_From_Sharearea(pTmp_Point->uiOffset, TYPE_YX_STATUS_BIT);  //“£–≈¡ø
            //tmp_buf[k+5] = (data&(1<<(pTmp_Point->uiOffset&0x7)))?1:0;
